...
export async function getAcommodationByPrice(req, res) {
  ...
  try {
    ...
    if (minPrice && maxPrice) {
      Acommodation = await db.query(
        `SELECT * FROM hotels WHERE price => $1 AND price <= $2`,
        [minPrice, maxPrice]
      );
    } else if (minPrice) {
      Acommodation = await db.query(`SELECT * FROM hotels WHERE price >= $1`, [
        minPrice,
      ]);
    } else if (maxPrice) {
      Acommodation = await db.query(`SELECT * FROM hotels WHERE price <= $1`, [
...
export async function getAllAcommodations (req,res){

try{
  const Acommodation = await db.query('SELECT * FROM hotels');
